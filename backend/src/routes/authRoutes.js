...
// Temporary store for reset tokens (for demo purposes)
const resetTokens = {};
...
router.post('/forgot-password', async (req, res) => {
    try {
        ...
        // Store token in memory (for demonstration)
...
router.post('/reset-password', async (req, res) => {
    try {
        ...
        if (!resetTokens[email] || resetTokens[email] !== token) {
            return res.status(400).json({ msg: 'Token does not match or expired' });
        }
